{% macro dbt_vectorize__embedding_generate(text_column, model='text-embedding-3-small', batch_size=128) -%}
    {#
...
{% macro dbt_vectors__embedding_generate(text_column, model='text-embedding-3-small', batch_size=128) -%}
    {{ return(dbt_vectorize__embedding_generate(text_column=text_column, model=model, batch_size=batch_size)) }}
{%- endmacro %}
